...
/*
 * LibGlade implementation of lookup_widget. Courtsey Bibek Sahu :).
 */
GtkWidget *lookup_widget_libglade (GtkWidget *widget, const gchar *widget_name)
{
	GladeXML *xml;
 ...
 */
GladeXML *
peacock_glade_dialog_new (const gchar *fname, const gchar *wname)
{
	GladeXML *gui;
	gchar *glade_file_path;

	glade_file_path = g_strdup_printf ("%s/%s", PACKAGE_GLADE_DIR, fname);
	gui = glade_xml_new (glade_file_path, wname);
